...
def novo_evento():
    if request.method=='POST':
        ...
        return redirect(url_for('construtor',evento_id=eid))
...
def construtor(evento_id):
    ...
    if request.method=='POST':
        ...
        conn.commit(); flash('Salvo com sucesso.','ok'); return redirect(url_for('construtor',evento_id=evento_id))
    campos=conn.execute('SELECT * FROM campos WHERE evento_id=? ORDER BY ordem,id',(evento_id,)).fetchall(); at=conn.execute('SELECT a.*, (SELECT COUNT(*) FROM inscricoes i WHERE i.atendimento_id=a.id) ocupadas FROM atendimentos a WHERE evento_id=? ORDER BY id',(evento_id,)).fetchall(); cv=conn.execute('SELECT * FROM campos_voluntarios WHERE evento_id=? ORDER BY ordem,id',(evento_id,)).fetchall(); conn.close()
    return render_template('construtor.html',evento=evento,campos=campos,atendimentos=at,campos_voluntarios=cv)
...
def cadastro_voluntario(evento_id):
    ...
    if not evento or not evento['ativo']: conn.close(); return render_template('publico_mensagem.html',mensagem='Este cadastro não está disponível no momento.')
    if request.method=='POST':
        cur=conn.execute('INSERT INTO voluntarios (evento_id) VALUES (?)',(evento_id,)); vid=cur.lastrowid
    ...
    conn.close(); return render_template('cadastro_voluntario.html',evento=evento,campos=campos)
...
def voluntarios(evento_id):
    conn=get_db(); evento=conn.execute('SELECT * FROM eventos WHERE id=?',(evento_id,)).fetchone(); campos=conn.execute('SELECT * FROM campos_voluntarios WHERE evento_id=? ORDER BY ordem,id',(evento_id,)).fetchall(); vs=conn.execute('SELECT * FROM voluntarios WHERE evento_id=? ORDER BY id DESC',(evento_id,)).fetchall(); linhas=[]
    for v in vs:
...
def exportar(evento_id):
    ...
    for row,i in enumerate(ins,1):
        mapa={r['campo_id']:r['valor'] for r in conn.execute('SELECT campo_id,valor FROM respostas WHERE inscricao_id=?',(i['id'],)).fetchall()}; vals=[i['id'],i['atendimento_nome'] or '']+[mapa.get(c['id'],'') for c in campos]+[i['criado_em'] or '']
